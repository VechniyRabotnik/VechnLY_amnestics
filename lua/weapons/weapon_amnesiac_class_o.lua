...
AddCSLuaFile()
SWEP.myclass = "weapon_amnesiac_class_o"
SWEP.classamenstic = "O" 
SWEP.DrawAmmo = true
...
function SWEP:Initialize()
    ...
    self.PrintName = translations["ClassO"] or "Amnestic | Class: O"
end
...
function SWEP:PrimaryAttack()
    ...
    if IsValid(Traced) and Traced:IsPlayer() or Traced:IsNPC() and !Traced:GetNWBool("amnestic_activated", false) then
        self:SetNextPrimaryFire(CurTime()+3)
        Owner:DoAnimationEvent(ACT_GMOD_GESTURE_MELEE_SHOVE_1HAND)
        if CLIENT then return end
        AmnesticsEffectLY(Owner, Owner, self.classamenstic)
